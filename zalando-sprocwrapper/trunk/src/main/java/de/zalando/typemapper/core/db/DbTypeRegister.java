...
public class DbTypeRegister {
    ...
    // this method was rewritten for better performance, so we don't have to calculate the fqName multiple times.
    // I kept the business logic, although we have wrong concepts here which should be fixed.
    private Map<String, String> buildTypeFQN(final Map<String, List<String>> typeNameToFQN) {
        ...
        for (final Entry<String, List<String>> entry : typeNameToFQN.entrySet()) {
            ...
            // Code smell: this code is here for backward compatibility reasons. This should be improved with time.
            // if the sproc specifies the schema of one type, we might end up using the wrong type (with same name
            // but different schema because we "resolve" the conflict using the search path which is not correct.
            // Main problems:
