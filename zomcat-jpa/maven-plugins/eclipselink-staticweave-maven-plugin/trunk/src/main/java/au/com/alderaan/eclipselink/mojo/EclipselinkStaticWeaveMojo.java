...
public class EclipselinkStaticWeaveMojo extends AbstractMojo {
     ...
     * {@link Class#forName(String)} without referencing a class loader that knows about project classes. Offending
     * method is {@link org.eclipse.persistence.internal.libraries.asm.ClassWriter#getCommonSuperClass(String, String)}
...
// CustomRealmClassLoader c = (CustomRealmClassLoader) Thread.currentThread().getContextClassLoader();
                List<URL> urls = buildClassPathList();
                urls.add(new File(project.getBuild().getOutputDirectory()).toURI().toURL());
/*                // Add Project output directory to class path.
 *              c.getClassRealm().addConstituent(new File(project.getBuild().getOutputDirectory()).toURI().toURL());
 *
 *              // Add Project class path to class path.
 *              for (URL url : buildClassPath()) {
 *                  c.getClassRealm().addConstituent(url);
 *              }*/
    ...
    public void setLogLevel(final String logLevel) {
        if (SessionLog.OFF_LABEL.equalsIgnoreCase(logLevel) || SessionLog.SEVERE_LABEL.equalsIgnoreCase(logLevel)
                ...
                || SessionLog.FINE_LABEL.equalsIgnoreCase(logLevel) || SessionLog.FINER_LABEL.equalsIgnoreCase(logLevel)
    ...
    protected List<URL> buildClassPathList() throws MalformedURLException {
