...
Grundlegende Unterstützung für die Verwendung von JPA bei Zalando. Ein Session-Customizer sorgt für die Zalando-Spezifische Benamung
von Tabellen und Spalten.

.Beispiel persistence.xml
[source,xml]
----
...
<persistence version="2.0"
    ...
    <persistence-unit name="default">
        ...
        <properties>
            ...
            <!-- ADVANCED ZALANDO-ECLIPSELINK-CONFIGURATION -->
            <!-- use deferred change tracking, because attribute change tracking, that is activated with weaving seems to be buggy -->
            <property name="zomcatJpa.changeTracker.type" value="DEFERRED"/>
        </properties>
    </persistence-unit>
</persistence>
----

Konfiguration der ChangePolicy
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Es wurden Probleme bei der Verwendung der AttributeChangeTrackingPolicy, welche teilweise automatisch durch static-weaving aktiviert wird, festgestellt.
Deshalb existiert hier ein ClassDescriptorCustomizer, der für alle Entity-Klassen die ChangeTrackingPolicy auf den definierten Wert setzt:
...
Mögliche Werte für _zomcatJpa.changeTracker.type_ sind:
...
Was diese im Detail bedeuten kann im http://wiki.eclipse.org/Introduction_to_EclipseLink_Transactions_%28ELUG%29#Unit_of_Work_and_Change_Policy[EclipseLink Wiki^] nachgelesen werden.
