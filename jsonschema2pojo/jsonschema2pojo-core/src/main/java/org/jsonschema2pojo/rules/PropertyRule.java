 ...
 *       http://tools.ietf.org/html/draft-zyp-json-schema-03#section-5.2</a>
 */
public class PropertyRule implements Rule<JDefinedClass, JDefinedClass> {

    private final RuleFactory ruleFactory;

    protected PropertyRule(final RuleFactory ruleFactory) {
     ...
     *
     * <p/>This rule adds a property to a given Java class according to the Java Bean spec. A private field is added to
     * the class, along with accompanying accessor methods.
     *
     * <p/>If this rule's schema mapper is configured to include builder methods (see
     * {@link org.jsonschema2pojo.GenerationConfig#isGenerateBuilders()}), then a builder method of the form <code>
     * withFoo(Foo foo);</code> is also added.
     *
     ...
     */
    @Override
    public JDefinedClass apply(final String nodeName, JsonNode node, final JDefinedClass jclass, final Schema schema) {
    ...
    private JsonNode resolveRefs(final JsonNode node, final Schema parent) {
        if (node.has("$ref")) {
    ...
    private boolean isObject(final JsonNode node) {
        return node.has("type") && node.get("type").asText().equals("object");
    }

    private boolean isArrayOfObjects(JsonNode node, final Schema parent) {
        node = resolveRefs(node.path("items"), parent);
        return node.path("type").asText().equals("object") || node.path("type").asText().equals("array");
    }

    private JMethod addGetter(final JDefinedClass c, final JFieldVar field, final String jsonPropertyName) {
    ...
    private JMethod addSetter(final JDefinedClass c, final JFieldVar field, final String jsonPropertyName) {
    ...
    private JMethod addBuilder(final JDefinedClass c, final JFieldVar field) {
    ...
    private String getGetterName(String propertyName, final JType type) {
