...
public class SchemaRule implements Rule<JClassContainer, JType> {
    ...
    protected SchemaRule(final RuleFactory ruleFactory) {
     ...
     *
     * <p/>At the root of a schema document this rule should be applied (schema documents contain a schema), but also in
     ...
     *
     * <p/>Where the schema value is a $ref, the ref URI is assumed to be applicable as a URL (from which content will
     * be read). Where the ref URI has been encountered before, the root Java type created by that schema will be
     * re-used (generation steps won't be repeated).
     *
     * @param  schema  the schema within which this schema rule is being applied
     */
    @Override
    public JType apply(final String nodeName, JsonNode schemaNode, final JClassContainer generatableType,
            Schema schema) {
