...
public class Arguments implements GenerationConfig {
    ...
    @Parameter(
        names = {"-t", "--target"}, description = "The target directory into which generated types will be written",
        required = true
    )
    ...
    @Parameter(
        names = {"-s", "--source"},
        description = "The source file(s) or directory(ies) from which JSON Schema will be read", required = true,
    ...
    @Parameter(
        names = {"-P", "--use-primitives"}, description = "Use primitives instead of wrapper types for bean properties"
    )
    ...
    @Parameter(
        ...
        description =
            "Use long (or Long) instead of int (or Integer) when the JSON Schema type 'integer' is encountered"
    )
    ...
    @Parameter(
        ...
        description =
            "The fully qualified class name of referring to a custom annotator class that implements org.jsonschema2pojo.Annotator "
    ...
    @Parameter(
        names = {"-303", "--jsr303-annotations"}, description = "Add JSR-303 annotations to generated Java types."
    )
    ...
    @Parameter(
        ...
        description =
            "Whether to empty the target directory before generation occurs, to clear out all source files that have been generated previously (indiscriminately deletes all files and folders)."
    )
     ...
     *
     * <p>If the command line arguments include the "help" argument, or if the arguments have incorrect values or order,
     * then usage information is printed to {@link System#out} and the program terminates.
     *
