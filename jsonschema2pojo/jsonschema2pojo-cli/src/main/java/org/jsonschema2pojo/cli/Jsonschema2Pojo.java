...
public final class Jsonschema2Pojo {
    ...
    /**
     * Main method, entry point for the application when invoked via the command line. Arguments are expected in POSIX
     * format, invoke with --help for details.
     *
     * @param   args  Incoming arguments from the command line
     *
     * @throws  FileNotFoundException  if the paths specified on the command line are not found
     * @throws  IOException            if the application is unable to read data from the paths specified
     */
     ...
     *
     * @param   config  the configuration options (including source and target paths, and other behavioural options)
     *                  that will control code generation
     *
     ...
     */
    public static void generate(final GenerationConfig config) throws FileNotFoundException, IOException {
        ...
        if (config.getTargetDirectory().exists() || config.getTargetDirectory().mkdirs()) {
            ...
            throw new GenerationException("Could not create or access target directory "
                    + config.getTargetDirectory().getAbsolutePath());
        }
    }

    private static void generateRecursive(final GenerationConfig config, final SchemaMapper mapper,
            final JCodeModel codeModel, final String packageName, final List<File> schemaFiles)
        ...
        for (File child : schemaFiles) {
            if (child.isFile()) {
                ...
                generateRecursive(config, mapper, codeModel, packageName + "." + child.getName(),
                    Arrays.asList(child.listFiles()));
            }
        }
    }

    private static void removeOldOutput(final File targetDirectory) {
    ...
    private static Annotator getAnnotator(final GenerationConfig config) {
    ...
    private static String getNodeName(final File file) {
